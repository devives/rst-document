...
public abstract class DirectiveBuilderAbst<
        PARENT extends RstNodeBuilder<?, ?, ?, ?>,
        RESULT extends DirectiveAbst<RstElement>,
        SELF extends RstElementBuilder<PARENT, RESULT, SELF>>
        extends RstElementBuilderAbst<PARENT, RESULT, SELF> {

    protected final Directive.Type type_;

    public DirectiveBuilderAbst(Directive.Type type) {
        type_ = type;
    }

    @Override
    protected RESULT createRstElement() {
        return (RESULT) Rst.elements().directive(type_);
    }

    protected SELF addArgument(String argument) {
        getRstElement().getArguments().add(argument);
        return (SELF) this;
    }
